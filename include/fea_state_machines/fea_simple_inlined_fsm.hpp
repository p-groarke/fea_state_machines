...
////
////	// Here, we use move semantics not for performance (it doesn't do
///anything). /	// It is to make it clear to the user he cannot modify the state
///anymore. /	// The fsm gobbles the state. /	template <StateEnum State> /
///void add_state(state_t&& state) { /		static_assert(State !=
///StateEnum::count, "fsm : bad state");
////
...
////	// Throws on bad transition (or asserts, if you defined
///FEA_FSM_NOTHROW). /	// If you had previously called delayed_trigger, this /
///// won't do anything. /	template <TransitionEnum Transition> /	void
///trigger(FuncArgs... func_args) { /		if (_has_delayed_trigger) /
///return;
////
